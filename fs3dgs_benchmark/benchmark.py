...
import csv
from fs3dgs_benchmark.bench_tables import BenchTables
from fs3dgs_benchmark.preprocess_handler import handle_dataset_preprocessing
import logging
...
def resolve_model_dir(model_name: str) -> Path:
    """
    Find model directory (FSGS, gaussian_splatting, DNGaussian)
    whether running from source or from installed package.
    """
    # Case 1: running from source checkout
    root = Path(__file__).resolve().parents[1]
    candidate = root / model_name
    if candidate.exists():
        return candidate

    # Case 2: running from installed source tarball
    # pip installs from .tar.gz unpack everything under site-packages/<pkg>-<ver>.dist-info/../
    for parent in Path(__file__).resolve().parents:
        maybe = parent / model_name
        if maybe.exists():
            return maybe

    raise FileNotFoundError(f"Could not locate model directory '{model_name}'")
...
def main():
    ...
    # Base output (parent)
    base_out_root = Path(cfg["defaults"].get("out_root", "./bench_runs")).expanduser().resolve()
    base_out_root.mkdir(parents=True, exist_ok=True)

    # Build run_id from date + config description
    run_desc = cfg["defaults"].get("run_desc", "")
    run_id = f"{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}_{_slug(run_desc)}" if run_desc else f"{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}"
    run_out_root = (base_out_root / run_id).resolve()
    run_out_root.mkdir(parents=True, exist_ok=True)

    # snapshot the config used
    try:
        shutil.copy2(args.config, run_out_root / "config.yaml")
    except Exception:
        pass

    # From here on, write ALL per-run artifacts under run_out_root
    ...
    for scene in scenes_resolved:
        ...
        for model in cfg.get("models", []):
            ...
            # # --- NEW: preprocess dataset if needed ---
            # try:
            #     updated_source_path = handle_dataset_preprocessing(model, scene, cfg["defaults"])
            #     scene["source_path"] = updated_source_path
            # except Exception as e:
            #     logging.error(f"[Preprocessing] Failed for {model_name}:{scene_name} — {e}")
            #     continue  # skip this model/scene if preprocessing fails
            
            # --- Sanity check ---
            print(f"[INFO] Final dataset path for {model_name}:{scene_name} → {scene['source_path']}")
    ...
    # ---- Write per-run summary CSV + generate LaTeX tables ----
    if not args.dry and summary_rows:
         # ensure this import is at top of file too

        # Normalize keys to avoid missing columns
        fieldnames = ["tag", "scene", "model", "seed",
                      "exit_code", "duration_sec", "psnr", "ssim", "lpips"]
        for rsum in summary_rows:
            for k in fieldnames:
                rsum.setdefault(k, None)

        # Write per-run summary inside this run's folder
        per_run_csv = out_root / "summary.csv"
        with open(per_run_csv, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=fieldnames)
            w.writeheader()
            w.writerows(summary_rows)
        print(f"[Summary] Wrote per-run summary to: {per_run_csv}")

        # Generate LaTeX + PNG tables for this run
        try:
            tables_dir = BenchTables(summary_rows).export(out_root)
            print(f"[Tables] Wrote LaTeX + PNG tables to: {tables_dir}")
        except Exception as e:
            print("[Tables] Skipped (export error):", e)
